...
namespace Mvc_ESM.Static_Helper
{
    class RoomArrangement
    {
        ...
        public static void Run()
        {
            Init();
            step2();
            AlgorithmRunner.WriteJson("SubjectTime", AlgorithmRunner.GroupsTime);
            AlgorithmRunner.WriteJson("GroupsRoom", AlgorithmRunner.GroupsRoom);
            AlgorithmRunner.WriteJson("GroupsRoomStudents", AlgorithmRunner.GroupsRoomStudents);
        }
        ...
        private static void Init()
        {
            ...
            for (int GroupIndex = 0; GroupIndex < AlgorithmRunner.Groups.Count; GroupIndex++)
            {
                ...
                IEnumerable<String> Result = db.Database.SqlQuery<String>("select sinhvien.MaSinhVien from pdkmh, sinhvien "
                                                                            ...
                                                                            + (IgnoreStudents.Length > 0 ? "and not(s2.MaSinhVien in (" + IgnoreStudents + ")) " : "")
        ...
        private static void step2()
        {
            ...
            GroupIndexList = GetNextStepGroups();//.ToList();
            ...
            while (GroupIndexList.Count() > 0)
            {
                ...
                {
                    RoomArrangementForOneGroup(si);
                }
                GroupIndexList = GetNextStepGroups();
            }
        }
        //- B3: Tăng thời gian của tất cả các môn có màu khác và thi sau lên 1 khoảng sao cho > max[màu các môn vừa tăng thời gian]
        private static void RoomArrangementForOneGroup(int GroupIndex)
        {
            int StudentsNumber = StudentByGroup[AlgorithmRunner.Groups[GroupIndex]].Count;
            ...
            while (StudentsNumber > 0)
            {
                ...
                {
                    AlgorithmRunner.GroupsRoom[GroupIndex].Add(InputHelper.Rooms[RoomUsedIndex]);
        ...
        private static List<int> GetNextStepGroups()
        {
            ...
            for (int i = 0; i < AlgorithmRunner.Groups.Count; i++)
            {
                if (!Progressed[i])
                {
                    DateTime StepTime = AlgorithmRunner.GroupsTime[i];
                    for (int si = 0; si < AlgorithmRunner.Groups.Count; si++)
                    {
                        if (!Progressed[si] && AlgorithmRunner.GroupsTime[si] == StepTime)
                        {
